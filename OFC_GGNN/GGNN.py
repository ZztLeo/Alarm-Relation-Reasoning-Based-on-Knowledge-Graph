...
class GGNN(nn.Module):
    def __init__(self,adjacency_matrix, hidden_state_channel=16, output_channel=5,
        ...
        self._mask = Variable(torch.zeros(self.num_classes, self.alarm_num),requires_grad=False).cuda()
        #print(self._mask)
    ...
    def split_matrix(self,matrix_file):
        ...
        in_matrix[:d_row, d_row:] = matrix_file
        out_matrix = np.zeros((d_row + d_col, d_row + d_col))
        out_matrix[d_row:, :d_row] = matrix_file.transpose()
